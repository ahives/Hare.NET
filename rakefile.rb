COPYRIGHT = "Copyright 2014 Albert L. Hives"
...
props = {
  ...
  :projects => ["Hare"],
  :keyfile => File.expand_path("Hare.snk")
}
...
assemblyinfo :global_version do |asm|
  ...
  asm.description = "Hare.NET is a .NET API built on top of HareDu to provide high level scenarios of configuring RabbitMQ via its REST API."
...
task :copy35 => [:build35] do
  copyOutputFiles File.join(props[:src], "Hare/bin/Release/v3.5"), "Hare.{dll,pdb,xml}", File.join(props[:output], 'net-3.5')
end

task :copy4 => [:build4] do
  copyOutputFiles File.join(props[:src], "Hare/bin/Release"), "Hare.{dll,pdb,xml}", File.join(props[:output], 'net-4.0-full')
end
...
nunit :tests35 => [:build35] do |nunit|
          ...
          nunit.assemblies = FileList[File.join(props[:src], "Hare.Tests/bin/Release", "Hare.Tests.dll")]
end
...
zip :zip_output => [:versioning] do |zip|
	...
	zip.output_file = "Hare-#{NUGET_VERSION}.zip"
	zip.output_path = props[:artifacts]
end

desc "Restore NuGet Packages"
task :nuget_restore do
  sh "#{props[:nuget]} install #{File.join(props[:src],"Hare","packages.config")} -Source https://nuget.org/api/v2/ -o #{File.join(props[:src],"packages")}"
  sh "#{props[:nuget]} install #{File.join(props[:src],"Hare.Tests","packages.config")} -Source https://nuget.org/api/v2/ -o #{File.join(props[:src],"packages")}"
...
task :nuget => [:versioning, :create_nuspec] do
  sh "#{props[:nuget]} pack #{props[:artifacts]}/Hare.nuspec /Symbols /OutputDirectory #{props[:artifacts]}"
end
...
nuspec :create_nuspec do |nuspec|
  ...
  nuspec.summary = 'Hare.NET is a .NET API built on top of HareDu to provide high level scenarios of configuring RabbitMQ via its REST API.'
  nuspec.description = 'Hare.NET is a .NET API built on top of HareDu to provide high level scenarios of configuring RabbitMQ via its REST API.'
  nuspec.title = 'Hare.NET'
  nuspec.projectUrl = 'http://github.com/ahives/Hare.NET'
  nuspec.language = "en-US"
  nuspec.licenseUrl = "http://www.apache.org/licenses/LICENSE-2.0"
  nuspec.requireLicenseAcceptance = "false"
  nuspec.dependency "HareDu", "1.1.0"
  ...
  nuspec.output_file = File.join(props[:artifacts], 'Hare.nuspec')
  add_files File.join(props[:output]), 'Hare.{dll,pdb,xml}', nuspec
  nuspec.file(File.join(props[:src], "Hare\\**\\*.cs").gsub("/","\\"), "src")
end
